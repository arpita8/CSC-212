 ...
 *
 *  @author Nicholas R. Howe and Zacharias J. Arthurs
 *  @version CSC 212, 18 April 2013
 */
public class Heap<E extends Comparable<E>> {
    ...
    private void bubbleDown() {
        ...
        boolean done = false;
        while (!done) {
            if (rightChild(pos) < heap.size() && isBigger(rightChild(pos), pos) && isBigger(rightChild(pos), leftChild(pos))) {
    ...
    public E popTop() {
        E topValue = peekTop();
        heap.setElementAt(heap.get(size() -1), 0);
        heap.remove(size() - 1);
        bubbleDown();   
        return topValue;
    }
